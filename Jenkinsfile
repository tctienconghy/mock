def runTerraform(environment) {
    ...
    def instanceIps = sh(
        script: "terraform -chdir=/var/jenkins/workspace/job-jenkins/env/${environment}/frontend/ apply --lock=false -auto-approve && terraform -chdir=/var/jenkins/workspace/job-jenkins/env/${environment}/frontend/ output list_ec2_ip",
        returnStdout: true
    ).trim()
    ...
    stages {
        ...
        stage('init') {
            steps {
                ...
                sh 'terraform -chdir=/var/jenkins/workspace/job-jenkins/env/${deployment_env}/frontend/ init --lock=false'
                }
            }
        }
        stage('validate') {
            steps {
                ...
                withAWS(credentials: 'my_aws_access', region: 'us-east-1') {
                sh 'terraform -chdir=/var/jenkins/workspace/job-jenkins/env/${deployment_env}/frontend/ validate'
                }
            }
        }
        stage('plan') {
            steps {
                echo "validate terraform with env: ${params.deployment_env}"
                withAWS(credentials: 'my_aws_access', region: 'us-east-1') {
                sh 'terraform -chdir=/var/jenkins/workspace/job-jenkins/env/${deployment_env}/frontend/ plan'
                }
            }
        }
        stage("run terraform"){
